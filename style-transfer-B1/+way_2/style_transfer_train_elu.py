...
test_path = '../examples/test_content'#测试图片所在路径

def weight_init(m):
    if isinstance(m, nn.Linear):
        m.weight.data.normal_(0, 0.02)#全部初始化为均值为0，方差为0.02的正态分布
        m.bias.data.zero_()#全部初始化为0
...
def content_loss(mse_loss, true_content, predicted_content):
    return mse_loss(true_content, predicted_content) * 0.5 / true_content.shape[0]
...
def training(my_vgg16, G_net, style_mse_loss, content_mse_loss, G_scheduler, G_optimizer, style_img, train_loader, transform, G_net_path):
    ...
    for epoch in range(0,5):
        for index, data in enumerate(train_loader,0):
            ...
            for _, key in enumerate(false_style):
                loss_style += style_loss(style_mse_loss, true_style[key], false_style[key])
            G_loss = 300 * loss_style + 1 * loss_content
            G_loss.backward()
            G_optimizer.step()
            ...
            if (index + 1) * 4 % 1000 == 0:
                epo = G_net_path.split('epoch ')[-1][0]
                iters = G_net_path.split('imgs num ')[-1].split('_Glr')[0]
                new_path = G_net_path.replace(iters, str((index + 1) * 4)).replace(epo, str(epoch + 1))
                state = {'model': copy.deepcopy(G_net.state_dict()), 'optimizer': copy.deepcopy(G_optimizer.state_dict()),
...
if __name__ == '__main__':
    ...
    style_path = '../examples/style/6.jpg'#可以改变序号来训练其他风格
    ...
    G_net_path = 'G_net_epoch 1_imgs num 16000_Glr 1e-4_style ' + str(style_path.split('.jpg')[0].split('/')[-1]) + '.pth'
    G_net = Reset_Generator(num_blocks=5)
    G_optimizer = optim.Adam([{'params': G_net.parameters(), 'initial_lr': 5e-5}], lr=5e-5, betas=(0.5, 0.999))
    G_scheduler = torch.optim.lr_scheduler.MultiStepLR(G_optimizer, milestones=[550], gamma=0.5, last_epoch=499)
    G_net.apply(weight_init)

    style_mse_loss = nn.MSELoss(reduction='mean')
    content_mse_loss = nn.MSELoss(reduction='sum')
